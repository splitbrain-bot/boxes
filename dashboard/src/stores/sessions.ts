 ...
 *
 * For a view that watches one session rather than the list: whether the
 * deployment holds a token is a fact about the deployment, so it is asked for
 * once on arrival instead of riding along with a list that view never reads.
 * A probe that did not answer leaves what is held, because a failed probe
 * says nothing about the token.
 */
export async function refreshHealth(): Promise<void> {
  try {
    const health = await api.health();
    set({ claudeTokenConfigured: health.claudeTokenConfigured, images: health.images });
  } catch {
